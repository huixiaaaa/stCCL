...
#
#         # # ===== 新增：可学习的损失权重参数（对数方差） =====
#         # self.log_contrast_weight = nn.Parameter(torch.tensor(np.log(0.2)))   # 初始值对应 3
#
#
#     def reset_parameters(self):
#         torch.nn.init.xavier_uniform_(self.weight1)
...
#
#     def forward(self, feat, feat_a, adj1, fadj):
#         ###空间视图1 radius方式
#         z1 = F.dropout(feat, self.dropout, self.training)
#         z1 = torch.mm(z1, self.weight1)
#         z1 = torch.mm(adj1, z1)
...
#         loss_ctr = loss_ctr1 + loss_ctr3
#         # ----- 重建损失 -----
#         loss_feat = F.mse_loss(feat, h)  # feat 是原始输入
#
#         # # ===== 自适应加权总损失（不确定性加权） =====
#         # contrast_weight = torch.exp(self.log_contrast_weight)   # 确保正数，初始≈0.2
#         # contrast_weight = torch.clamp(contrast_weight, 0.01, 1.0)  # 限制最大为1，防止过大
#         # total_loss = loss_feat + contrast_weight * loss_ctr
#         # contrast_weight=0.2
#         # total_loss = loss_feat + contrast_weight * loss_ctr
#         # 返回：隐表示、重建、各损失分量、总损失
#         return co_hiden_emb, h, loss_feat, loss_ctr1, loss_ctr3
#
