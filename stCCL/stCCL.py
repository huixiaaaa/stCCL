...
#     contrast_weight = 0.2
#     ema_alpha = 0.99
#     weight_min, weight_max = 0.05, 0.8
#     for epoch in tqdm(range(arg.epoch)):
#         model.train()
#         features_a = permutation(features)
#         # ===== 模型返回 5 个值 =====
#         co_hiden_emb, h, loss_feat, loss_ctr1, loss_ctr3 = model(features, features_a, adj1, fadj)
#         # loss_ctr=loss_ctr1+loss_ctr3
#         # 每隔一定步数更新权重
#         if epoch % 10 == 0 and epoch > 0:
#             ratio = loss_feat.item() / (loss_ctr1.item() + loss_ctr3.item() + 1e-8)
#             # 希望对比项贡献 = 0.2 * loss_feat
#             target_weight = 0.2 * ratio  # 因为 contrast_weight * loss_ctr = 0.2 * loss_feat => weight = 0.2 * loss_feat / loss_ctr = 0.2 * ratio
#             target_weight = max(weight_min, min(weight_max, target_weight))
#             contrast_weight = ema_alpha * contrast_weight + (1 - ema_alpha) * target_weight
#
#         total_loss = loss_feat + contrast_weight * (loss_ctr1 + loss_ctr3)
#         # total_loss = loss_feat + contrast_weight * loss_ctr
#         # total_loss =loss_feat+0.2*(loss_ctr1+loss_ctr3)
...
#                     print(' epoch: ', epoch, ' feat_loss = {:.2f}'.format(loss_feat),
#                           ' loss_sl_1 = {:.2f}'.format(loss_ctr1), ' loss_sl_2 = {:.2f}'.format(loss_ctr3),
#                           ' total_loss = {:.2f}'.format(total_loss), ' contrast_weight = {:.2f}'.format(contrast_weight))
#         optimizer.zero_grad()
#         total_loss.backward()
#         optimizer.step()
#
...
#     with torch.no_grad():
#         model.eval()
#         # latent_rep, reconst_rep, _ ,_= model(features, features_a, adj1, fadj)
#         latent_rep, reconst_rep, _, _, _= model(features, features_a, adj1, fadj)
#         latent = latent_rep.to('cpu').detach().numpy()
#         reconst_rep = reconst_rep.to('cpu').detach().numpy()
#         adata.obsm['latent'] = latent
#         adata.obsm['stCCL'] = reconst_rep
